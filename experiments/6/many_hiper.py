...
hiperparameters = [[0.01, 1 + 750/N, 5.0, 2.5, 20.0],  
...
for i in range(len(hiperparameters)):
    label_value = fr"$\epsilon = ${hiperparameters[i][0]}, $\epsilon_\delta = ${np.round((hiperparameters[i][1] - 1) * N,2)}, $\delta_L = ${hiperparameters[i][2]}, $\sigma = ${hiperparameters[i][3]}, $\delta_T = ${hiperparameters[i][4]}"
    ### run q-learning
    N=int(5e5)
    seed = 0
    np.random.seed(seed)
    ...
    details = Experiment_run(details, N, q0, q1, n0, n1, betas_grid, alpha, hiperparameters[i], N0=int(5e5))
