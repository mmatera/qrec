...
def Circuit(state, params, noise_params):
    state = qp.ry(noise_params[0]) * state
    state = qp.rz(noise_params[1]) * state
    state = qp.rx(params[0]) * state
    return state
...
for i in range(len(thetas)):
    ...
    model_scores.append(Score_function(state))
for i in range(len(qlearn.q0)):
    qlearn.q0[i] = model_scores[i]

noise_params = [1.2, 0.75]

for i in range(N):
    if i % (N // 10) == 0:
        print(i)
    beta_indx, beta, outcome, reward = Training(
        qlearn, betas_grid, hiperparam, epsilon=0.05, noise_params=noise_params
    )
...
plt.plot(thetas, model_scores, label="No noise scores")
plt.xlabel(r"$\theta$")
plt.ylabel("P(-1)")
plt.legend()
plt.show()
