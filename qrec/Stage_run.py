...
def Update_reload(n0, n1, epsilon, mean_rew, restart_point):
    epsilon = (
        0.15  # Example, but important to see the maximum reward and current reward.
    )
...
def Reset_with_model(alpha, beta_grid, q0, q1, n0, n1, restart_value):
    for i in range(len(beta_grid)):
        vals = 0.5 * np.exp(-np.abs(-alpha - beta_grid[i]) ** 2) + 0.5 * (
            1 - np.exp(-np.abs(alpha - beta_grid[i]) ** 2)
        )
        q0[i] = vals
    ...
    for i in range(len(q1)):
        for j in range(len(q1[i])):
            for k in range(len(q1[i, j])):
                q1[i, j, k] = p((-1) ** (k + 1) * alpha - beta_grid[i], j) / (
                    p(-alpha - beta_grid[i], j) + p(alpha - beta_grid[i], j)
                )
...
def Experiment_run(
    details, N, q0, q1, n0, n1, betas_grid, alpha, hiperparam=[], delta1=500
):
    start = time.time()
    mean_rew = 0
    points = [1, 1]
    ...
    for experiment in range(0, N):
        ...
        indg, g = ep_greedy(
            q1[indb, n, :], [0, 1], hiperparam[2], hiperparam[3], ep=epsilon
        )
        ...
        if experiment % delta1 == 0:
            ...
            if (
                mean_deriv < -0.001 and n0[indb] > 300
            ):  # if the change is big enough change the learning rate and epsilon.
                n0, n1, epsilon = Update_reload(
                    n0, n1, epsilon, mean_rew, hiperparam[4]
                )
        ...
        details["Ps_greedy"].append(
            Psq(q0, q1, betas_grid, hiperparam[2], hiperparam[3], alpha=alpha)
        )
        ...
        if Checked:
            ...
            if len(exp_vals["Phase"]) >= model_tries:
                ...
                if np.abs(guessed_intensity - current) > 0.3:
                    ...
                    q0, epsilon, n0 = Reset_with_model(
                        current, betas_grid, q0, q1, n0, n1, hiperparam[4]
                    )
                Checked = False
                exp_vals["Phase"] = []
                exp_vals["Betas"] = []
