...
# from qrec.Model_Semi_aware.find_unobservables import Find_optimal_intensity
...
def updates(indb, n, g, r, q0, q1, n0, n1, lr=0.001):
    ...
    q0[indb] += (1/n0[indb])*np.max([q1[indb, n, g]
                                     for g in [0, 1]] - q0[indb])
    n0[indb] += 1
    n1[indb, n, g] += 1
    return q0, q1, n0, n1
...
def reset_with_model(alpha, beta_grid, qlearn: Qlearn_parameters):
    """
    ...
    for i, q1_i in enumerate(q_1):
        for outcome, q1_ij in enumerate(q1_i):
            for k, q1_ijk in enumerate(q1_ij):
                beta = -beta_grid[i]
                prob = p_model((-1)**(k+1) * alpha, beta, outcome)

                # Marginal probability of the outcome
                # for unknown phase of alpha
                total_prob = (p_model(-alpha, beta, outcome) +
                              p_model(alpha, beta, outcome))
...
def Experiment_noise_1(qlearn: Qlearn_parameters, betas_grid, hiperparam, epsilon, alpha, lambd):
    q_0 = qlearn.q0
    q_1 = qlearn.q1
    hidden_phase = np.random.choice([0, 1])

    indb, b = ep_greedy(q0, betas_grid, hiperparam[2],
                        nr_prob=epsilon)
    ...
    for experiment in range(0, N):
        ...
        if noise_type == 1:
            indb, b, n, indg, g, r = Experiment_noise_1(
                q0, q1, betas_grid, hiperparam, epsilon, alpha, lambd)
        if noise_type == 2:
        ...
        if experiment % delta1 == 0:
            ...
            if mean_deriv <= -0.2 and n0[indb] > 300:
                n0, n1, epsilon = Update_reload(
                    n0, n1, mean_rew, hiperparam[3], hiperparam[0])
                Checked = True
        ...
        min, pstar, bstar = model_aware_optimal(
        ...
        details["Ps_greedy"].append(
            Psq(q0, q1, betas_grid, hiperparam[2], alpha=alpha, lambd=lambd))
