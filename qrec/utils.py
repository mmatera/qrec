...
Hyperparameters = namedtuple("Hyperparameters", [
    "eps_0",  # Epsilon0
    "deps_0",  # epsilon0 change rate
    "delta",  # Dispersion of the random gaussian
    "reset_lr"  # Learning rate reset
]
)
Qlearn_parameters = namedtuple("Qlearn_parameters ", [
    "q0",
    "q1",
    "n0",
    "n1"])
...
def bayes_decision_error_probability(beta,
                                     alpha=0.4,
                                     noise_val=0.0,
                                     noise_type=1):
    """
    ...
    if noise_type == 1:
        ...
        detune = 0
    return 1 - sum(
        max(prob(sgn*alpha, beta, detune, outcome) * prior
...
def model_aware_optimal(betas_grid, alpha=0.4, lambd=0.0, noise_type=1):
    ...
    mmin = minimize(bayes_decision_error_probability,
                    x0=-alpha,
...
def near_random(q_0, delta1):
    ...
    """
    maximum = max(q_0)
    weight = np.array([gaussian_values(q0_i, maximum, delta1)
                      for q0_i in q_0])
...
def ep_greedy(qvals, actions, delta1, nr_prob=1.):
    """
...
def give_outcome(hidden_phase, beta, alpha=0.4, lambd=0.0):
    ...
    sgn = (-1)**hidden_phase
    values = np.array([0, 1])
    p_0 = detection_state_probability(alpha*sgn,
                                      beta,
                                      lambd,
                                      0)
...
def comm_success_prob(hyperparms, betas_grid, dispersion, alpha=0.4, detuning=0.0):
    """
    (Former Psq)
    ...
    indb, beta = ep_greedy(q_0, betas_grid, dispersion, nr_prob=nr_prob)[1]
    ...
    def alpha_from_experiment(out):
        ...
        to the parameters
        """
        return ep_greedy(q_1[indb, out, :],
                         alpha_phases, dispersion, nr_prob=nr_prob)[1]
